...
(function(a){Mavo.UI.Message=a.Class({constructor:function(b,c,d={}){if(this.mavo=b,this.message=c,this.closed=Mavo.promise(),this.element=a.create({className:"mv-ui mv-message"+(d.type?" mv-"+d.type:""),["string"==a.type(this.message)?"innerHTML":"contents"]:this.message,events:{click:()=>Mavo.scrollIntoViewIfNeeded(this.mavo.element)},[this.mavo.bar?"after":"start"]:(this.mavo.bar||this.mavo).element}),d.classes&&this.element.classList.add(...d.classes.split(/\s+/)),"error"==d.type?this.element.setAttribute("role","alert"):this.element.setAttribute("aria-live","polite"),d.dismiss=d.dismiss||{},"string"==typeof d.dismiss||Array.isArray(d.dismiss)){var e={};Mavo.toArray(d.dismiss).forEach((a)=>{e[a]=!0}),d.dismiss=e}if(d.dismiss.button&&a.create("button",{className:"mv-close mv-ui",textContent:"\xD7",events:{click:()=>this.close()},start:this.element,title:this.mavo._("dismiss")}),d.dismiss.timeout){var f,g="number"==typeof d.dismiss.timeout?d.dismiss.timeout:5e3;a.bind(this.element,{mouseenter:()=>clearTimeout(f),mouseleave:Mavo.rr(()=>f=setTimeout(()=>this.close(),g))})}d.dismiss.submit&&this.element.addEventListener("submit",(a)=>{a.preventDefault(),this.close(a.target)})},close:function(b){a.transition(this.element,{opacity:0}).then(()=>{a.remove(this.element),this.closed.resolve(b)})}})})(Bliss,Bliss.$);
...
(function(a,b){var c=Mavo.Node=a.Class({abstract:!0,constructor:function(b,d,e={}){if(!b||!d)throw new Error("Mavo.Node constructor requires an element argument and a mavo object");var f={context:this,options:e};this.uid=++c.maxId,this.nodeType=this.nodeType,this.property=null,this.element=b,a.extend(this,f.options),c.all.set(b,[...(c.all.get(this.element)||[]),this]),this.mavo=d,this.group=this.parent=this.parentGroup=f.options.group,this.template=f.options.template,this.alias=this.element.getAttribute("mv-alias"),this.template?this.template.copies.push(this):this.copies=[],this.fromTemplate("property","type","storage","path")||(this.property=c.getProperty(b),this.type=Mavo.Group.normalize(b),this.storage=this.element.getAttribute("mv-storage"),this.path=this.getPath()),this.modes=this.element.getAttribute("mv-mode"),Mavo.hooks.run("node-init-start",f),this.mode=Mavo.getStyle(this.element,"--mv-mode")||"read",this.collection=f.options.collection,this.collection&&(this.group=this.parentGroup=this.collection.parentGroup);var g=this.template;if(g&&g.expressions&&(this.expressions=g.expressions.map((a)=>new Mavo.DOMExpression({template:a,item:this,mavo:this.mavo}))),this instanceof Mavo.Group||this.collection){var h=Mavo.DOMExpression.search(this.element).filter((a)=>"mv-value"==a.originalAttribute)[0];h&&(h.mavoNode=this,this.expressionText=h,this.storage=this.storage||"none",this.modes="read",this.collection&&(this.collection.expressions=[...(this.collection.expressions||[]),h],h.mavoNode=this.collection,this.collection.storage=this.collection.storage||"none",this.collection.modes="read"))}Mavo.hooks.run("node-init-end",f)},get editing(){return"edit"==this.mode},get isRoot(){return!this.property},get name(){return Mavo.Functions.readable(this.property||this.type).toLowerCase()},get saved(){return"none"!==this.storage},postInit:function(){"edit"==this.modes&&this.edit()},destroy:function(){this.template&&Mavo.delete(this.template.copies,this),this.expressions&&this.expressions.forEach((a)=>a.destroy()),this.itembar&&this.itembar.destroy()},getData:function(a={}){if(this.isDataNull(a))return null},getLiveData:function(){return this.isDataNull({live:!0})?this.collection?Mavo.objectify(null):null:this.liveData[Mavo.toProxy]},updateParentLiveData:function(a){if(void 0===a&&(a=Mavo.in(this.liveData,Mavo.toProxy)?this.liveData[Mavo.toProxy]:this.liveData),this.collection instanceof Mavo.ImplicitCollection)this.collection.updateLiveData();else{var b=this.collection?this.index:this.property;this.parent.liveData[b]=a}},isDataNull:function(a={}){var b={context:this,options:a,result:!this.saved&&!a.live};return Mavo.hooks.run("unit-isdatanull",b),b.result},walk:function(a,b=[],c={}){var d=(b,e)=>{var f=a(b,e);if(!1!==f)for(let a in b.children){let g=b.children[a];if(g instanceof Mavo.Node){var f=d.call(g,g,[...e,a]);if(!1===f&&!c.descentReturn)return!1}}return!1!==f};return d(this,b)},walkUp:function(a){for(var b=this;b=b.parentGroup;){var c=a(b);if(c!==void 0)return c}},edit:function(){return this.mode="edit","edit"==this.mode&&void(a.fire(this.element,"mv-edit",{mavo:this.mavo,node:this}),Mavo.hooks.run("node-edit-end",this))},done:function(){return this.mode=Mavo.getStyle(this.element.parentNode,"--mv-mode")||"read","read"==this.mode&&void(a.unbind(this.element,".mavo:edit"),a.fire(this.element,"mv-done",{mavo:this.mavo,node:this}),this.propagate("done"),Mavo.hooks.run("node-done-end",this))},save:function(){this.unsavedChanges=!1},propagate:function(a){for(let b in this.children){let c=this.children[b];c instanceof Mavo.Node&&("function"==typeof a?a.call(c,c):a in c&&c[a]())}},propagated:["save","destroy"],toJSON:Mavo.prototype.toJSON,fromTemplate:function(...a){return this.template&&a.forEach((a)=>this[a]=this.template[a]),!!this.template},render:function(a){var b=Mavo.in(a,Mavo.toNode);b&&(a=Mavo.clone(a)),this.oldData=this.data,this.data=a,b||(a=Mavo.subset(a,this.inPath));var c={context:this,data:a};if(Mavo.hooks.run("node-render-start",c),!/Collection$/.test(this.nodeType)&&Array.isArray(c.data)){var d;this.isRoot&&1===(d=this.getNames("Collection")).length?c.data={[d[0]]:c.data}:(this.inPath.push("0"),c.data=c.data[0])}else this.childrenNames&&1==this.childrenNames.length&&this.childrenNames[0]===this.property&&null!==c.data&&"object"==typeof c.data&&(c.data=c.data[this.property]);var e=this.editing;e&&this.done(),this.dataRender(c.data,b),e&&this.edit(),this.save(),Mavo.hooks.run("node-render-end",c)},dataChanged:function(b,c={}){var d=a.extend({action:b,property:this.property,mavo:this.mavo,node:this},c);a.fire(c.element||this.element,"mv-change",d),this.mavo.changed(d)},toString:function(){return`#${this.uid}: ${this.nodeType} (${this.property})`},getClosestCollection:function(){var a=this.closestItem;return a?a.collection:null},getClosestItem:function(){return this.collection&&/Collection$/.test(this.collection.nodeType)?this:this.parentGroup?this.parentGroup.closestItem:null},getPath:function(){var a=this.parent?this.parent.path:[];return this.property?[...a,this.property]:a},resolve:function(a,b={}){if(!("string"!=typeof a||-1<["Mavo"].indexOf(a))){var c=this.find(a,b);if(void 0===c){var d=-1<this.path.slice(0,-1).indexOf(a);c=this.walkUp((c)=>{if(c.property==a)return c;if(!d){if(a in c.children)return c.children[a];if(c.isRoot)return c.find(a,b)}})}return c}},resolveAgainstData:function(a,b){if(a in b)return b[a];if(a in c.special)return c.special[a].call(this);var d=this.resolve(a);return void 0===d?a in Mavo.all&&isNaN(a)&&Mavo.all[a].root?Mavo.all[a].root.getLiveData():Mavo.Functions._Trap[a]:(Array.isArray(d)?(d=d.map((a)=>a.getLiveData()),!Mavo.Actions.running&&(d=d.filter((a)=>null!==Mavo.value(a)))):d instanceof Mavo.Node&&(d=d.getLiveData()),d)},relativizeData:self.Proxy?function(a){return new Proxy(a,{get:(a,b)=>{return b in a?a[b]:this.resolveAgainstData(b,a)},has:(a,b)=>{var c=this.resolveAgainstData(b,a);return c!==void 0},set:function(a,b="",c){return console.warn(`You cannot set data via expressions. Attempt to set ${b.toString()} to ${c} ignored.`),c}})}:(a)=>a,createLiveData:function(a={}){return a[Mavo.toNode]=this,a[Mavo.toProxy]=this.relativizeData(a),a},pathFrom:function(a){for(var b=this.path,c=a.path,d=0;d<b.length&&c[d]==b[d];d++);return b.slice(d)},getDescendant:function(a){return a.reduce((a,b)=>a.children[b],this)},getCousin:function(a,b={}){if(!this.closestCollection)return null;var c=this.closestCollection,d=Math.abs(a),e=0>a?-1:1;if(c.length<d+1)return null;var f=this.closestItem.index+a;b.wrap&&(f=Mavo.wrap(f,c.length));for(var g,h=0;h<c.length;h++){g=f+h*e,b.wrap&&(g=Mavo.wrap(g,c.length));var i=c.children[g];if(i)break}if(!i||i==this.closestItem)return null;if(this.collection)return i;var j=this.pathFrom(this.closestItem);return i.getDescendant(j)},contains:function(a){do{if(a===this)return!0;a=a.parent}while(a);return!1},lazy:{closestCollection:function(){return this.getClosestCollection()},closestItem:function(){return this.getClosestItem()},inPath:function(){var a="Collection"==this.nodeType?"mv-multiple-path":"mv-path";return(this.element.getAttribute(a)||"").split("/").filter((a)=>a.length)},properties:function(){if(this.template)return this.template.properties;var a=new Set(this.property&&[this.property]);if("Group"==this.nodeType)for(var b in this.children)a=Mavo.union(a,this.children[b].properties);else"Collection"==this.nodeType&&(a=Mavo.union(a,this.itemTemplate.properties));return a}},live:{store:function(b){a.toggleAttribute(this.element,"mv-storage",b)},unsavedChanges:function(a){return!a||this.saved&&this.editing||(a=!1),/Collection$/.test(this.nodeType)||this.element.classList.toggle("mv-unsaved-changes",a),a},mode:function(b){if(this._mode!=b){if(this.modes&&b!=this.modes&&(b=this.modes),this._mode=b,!/Collection$/.test(this.nodeType)&&-1<[null,"","read","edit"].indexOf(this.element.getAttribute("mv-mode"))){var c=this.modes||"edit"==b;Mavo.Observer.sneak(this.mavo.modeObserver,()=>{a.toggleAttribute(this.element,"mv-mode",b,c)})}return b}},modes:function(a){return a&&"read"!=a&&"edit"!=a?null:void(this._modes=a,a&&this.mode!=a&&(this.mode=a))},collection:function(a){this.parent=a||this.parentGroup}},static:{maxId:0,all:new WeakMap,create:function(a,b,c={}){return Mavo.is("multiple",a)&&!c.collection?new Mavo.Collection(a,b,c):new Mavo[Mavo.is("group",a)?"Group":"Primitive"](a,b,c)},getProperty:function(a){var b=a.getAttribute("property")||a.getAttribute("itemprop");return b||(a.hasAttribute("property")?b=a.name||a.id||a.classList[0]:a.matches(Mavo.selectors.multiple)&&(b=a.getAttribute("mv-multiple")||"collection")),b&&a.setAttribute("property",b),b},get:function(a,b){var d=(c.all.get(a)||[]).filter((a)=>!/Collection$/.test(a.nodeType));return 2>d.length||!b?d[0]:d[0]instanceof Mavo.Group?node[1]:void 0},getClosest:function(a,b){var d;do d=c.get(a,b);while(!d&&(a=a.parentNode));return d},getClosestItem:function(a){var b=c.getClosest(a);return b instanceof Mavo.Primitive&&!b.collection?b.parent:b},children:function(a){var c=Mavo.Node.get(a);return c?[c]:(c=b(Mavo.selectors.property,a).map((a)=>Mavo.Node.get(a)).filter((b)=>!a.contains(b.parentGroup.element)).map((a)=>a.collection||a),Mavo.Functions.unique(c))},special:{$index:function(){return this.closestItem?this.closestItem.index:-1},$item:function(){return this.closestItem?this.closestItem.getLiveData():null},$all:function(){return this.closestCollection?this.closestCollection.getLiveData():null},$next:function(){var a=c.special.$all.call(this);return a?a[this.closestItem.index+1]:null},$previous:function(){var a=c.special.$all.call(this);return a?a[this.closestItem.index-1]:null}}}})})(Bliss,Bliss.$);
(function(a,b){var c=Mavo.Group=a.Class({extends:Mavo.Node,nodeType:"Group",constructor:function(){if(this.children={},this.group=this,Mavo.hooks.run("group-init-start",this),this.liveData=this.createLiveData(),this.parent&&!this.collection&&(this.parent.liveData[this.property]=this.liveData[Mavo.toProxy]),Mavo.Primitive.getValueAttribute(this.element))this.children[this.property]=new Mavo.Primitive(this.element,this.mavo,{group:this});var a=b(Mavo.selectors.property+", "+Mavo.selectors.multiple,this.element).filter((a)=>{return this.element===a.parentNode.closest(Mavo.selectors.group)}),c={};a.forEach((a)=>{var b=Mavo.Node.getProperty(a);"multiple"!==c[b]&&(c[b]=Mavo.is("multiple",a)?"multiple":(c[b]||0)+1)}),a.forEach((a)=>{var b=Mavo.Node.getProperty(a),d=this.template?this.template.children[b]:null,e={template:d,group:this},f=c[b];if("multiple"===f){var g=this.children[b];g instanceof Mavo.Collection?g.add(a):Mavo.is("multiple",a)?(this.children[b]=new Mavo.Collection(a,this.mavo,e),(g||[]).forEach((a,c)=>this.children[b].add(a,c))):this.children[b]=[...(g||[]),a]}else 1<f?this.children[b]?this.children[b].add(a):this.children[b]=new Mavo.ImplicitCollection(a,this.mavo,e):this.children[b]=Mavo.Node.create(a,this.mavo,e)}),this.childrenNames=Object.keys(this.children);var d=(this.isRoot?this.element.closest("[vocab]"):null)||this.element;this.vocab=d.getAttribute("vocab"),this.postInit(),Mavo.hooks.run("group-init-end",this)},get isRoot(){return!this.property},getNames:function(a="Node"){return Object.keys(this.children).filter((b)=>this.children[b]instanceof Mavo[a])},getData:function(a={}){var b={context:this,options:a,data:this.super.getData.call(this,a)};if(void 0!==b.data)return b.data;for(var d in b.data=Mavo.shallowClone(Mavo.subset(this.data,this.inPath))||{},this.children){var e=this.children[d];if(e.saved)var f=e.getData(b.options);e.saved&&null!==Mavo.value(f)?b.data[e.property]=f:delete b.data[e.property]}return this.childrenNames.length||this.isRoot?1===this.childrenNames.length&&this.property in this.children?b.data=b.data[this.property]:b.data&&"object"==typeof b.data&&(this.type&&this.type!=c.DEFAULT_TYPE&&(b.data["@type"]=this.type),this.vocab&&(b.data["@context"]=this.vocab)):b.data=null,b.data=Mavo.subset(this.data,this.inPath,b.data),Mavo.hooks.run("node-getdata-end",b),b.data},find:function(a,b={}){if(b.exclude!==this){if(this.property==a)return this;if(a in this.children)return this.children[a].find(a,b);if(this.properties.has(a)){var c,d,e=[];for(var f in this.children)d=this.children[f].find(a,b),void 0!==d&&(Array.isArray(d)?(e.push(...d),c=!0):e.push(d));return c||1<e.length?e:e[0]}}},edit:function(a={}){return!1!==this.super.edit.call(this)&&Promise.all(Object.keys(this.children).map((b)=>this.children[b].edit(a)))},propagated:["save","import"],dataRender:function(b){if(b){if("object"!=typeof b){if(this.property in this.children)var c=this.property;else var d=a.type(b),e=(a)=>(this.children[a]instanceof Mavo.Primitive)+(this.children[a].datatype==d),c=Object.keys(this.children).filter((a)=>!this.children[a].expressionText).sort((a,b)=>e(a)-e(b)).reverse()[0];b={[c]:b},this.data=Mavo.subset(this.data,this.inPath,b)}var f;this.propagate((c)=>{var d=b[c.property];c.alias&&void 0!==b[c.alias]&&(f=f||a.extend({},b),d=b[c.alias]),c.render(d)}),f&&this.propagate((a)=>{a.alias&&(b[a.property]=f[a.alias],!(a.alias in this.children)&&delete b[a.alias])});var c}},static:{all:new WeakMap,DEFAULT_TYPE:"Item",normalize:function(a){if(Mavo.is("group",a)){var b=Mavo.getAttribute(a,"typeof","itemtype")||c.DEFAULT_TYPE;return a.setAttribute("typeof",b),b}return null}}})})(Bliss,Bliss.$);
(function(a,b){var c=Mavo.Primitive=a.Class({extends:Mavo.Node,nodeType:"Primitive",constructor:function(d){this.fromTemplate("config","attribute","templateValue","originalEditor")||(this.config=c.getConfig(d),this.attribute=this.config.attribute),this.datatype=this.config.datatype,"modes"in this.config&&(this.modes=this.config.modes,this.element.setAttribute("mv-mode",this.config.modes)),Mavo.hooks.run("primitive-init-start",this),this.expressionText=this.expressionText||Mavo.DOMExpression.search(this.element,this.attribute),this.expressionText&&!this.expressionText.mavoNode&&(this.expressionText.primitive=this,this.storage=this.storage||"none",this.modes="read",this.element.setAttribute("aria-live","polite")),!this.editor&&this.element.hasAttribute("mv-edit")&&(!this.originalEditor&&(this.originalEditor=a(this.element.getAttribute("mv-edit"))),this.originalEditor&&!this.template&&(this.originalEditorObserver=new Mavo.Observer(this.originalEditor,"all",()=>{this.copies.concat(this).forEach((a)=>{"editor"==a.defaultSource&&(a.default=this.originalEditor.value),a.editor&&(a.editor=this.originalEditor.cloneNode(!0)),a.setValue(a.value,{force:!0,silent:!0})})}))),this.editor||this.originalEditor||this.attribute||(this.editor=b(this.element.children).filter(function(a){return a.matches(Mavo.selectors.formControl)&&!a.matches(Mavo.selectors.property)})[0],this.editor&&a.remove(this.editor));var e=this.editorValue;this.datatype||"number"!=typeof e&&"boolean"!=typeof e||(this.datatype=typeof e),this.config.init&&this.config.init.call(this,this.element),this.config.changeEvents&&a.bind(this.element,this.config.changeEvents,(a)=>{a.target===this.element&&(this.value=this.getValue())}),this.templateValue=this.getValue(),this._default=this.element.getAttribute("mv-default"),null===this.default?(this._default=this.modes?this.templateValue:e,this.defaultSource=this.modes?"template":"editor"):""===this.default?(this._default=this.templateValue,this.defaultSource="template"):(this.defaultObserver=new Mavo.Observer(this.element,"mv-default",()=>{this.default=this.element.getAttribute("mv-default")}),this.defaultSource="attribute");var f=!this.template||this.template.templateValue!=this.templateValue||"edit"==this.modes;this.initialValue=void 0===this.default&&f?this.templateValue:this.default,this.initialValue===void 0&&(this.initialValue=this.emptyValue),this.setValue(this.initialValue,{silent:!0}),Mavo.setAttributeShy(this.element,"aria-label",this.label),this.attribute||Mavo.setAttributeShy(this.element,"mv-attribute","none"),!1!==this.config.observer&&(this.observer=new Mavo.Observer(this.element,this.attribute,()=>{this.observer.running&&(this.attribute||!this.editing||this.config.subtree)&&(this.value=this.getValue())},{subtree:this.config.subtree,childList:this.config.subtree})),this.postInit(),Mavo.hooks.run("primitive-init-end",this)},get editorValue(){var b=this.editor||this.originalEditor;if(b){if(b.matches(Mavo.selectors.formControl))return c.getValue(b,{datatype:this.datatype});var d=a(Mavo.selectors.output+", "+Mavo.selectors.formControl,b);if(d)return c.getValue(d)}},set editorValue(b){if(this.config.setEditorValue&&"boolean"!==this.datatype)return this.config.setEditorValue.call(this,b);if(this.editor)if(this.editor.matches(Mavo.selectors.formControl))c.setValue(this.editor,b,{config:this.editorDefaults});else{var d=a(Mavo.selectors.output+", "+Mavo.selectors.formControl,this.editor);d&&c.setValue(d,b)}},destroy:function(){this.super.destroy.call(this),this.defaultObserver&&this.defaultObserver.destroy(),this.observer&&this.observer.destroy()},updateLiveData:function(){var a=this.value;this.isDataNull({live:!0})&&(a=null),this.liveData=this.createLiveData(Mavo.objectify(a)),this.collection&&(this.liveData[this.property]=this.liveData[Mavo.toProxy]),this.updateParentLiveData()},getLiveData:function(){return void 0===this.liveData&&this.updateLiveData(),this.liveData&&"object"==typeof this.liveData?(this.liveData[Mavo.toProxy]||(this.liveData=this.createLiveData(this.value)),this.liveData[Mavo.toProxy]):this.liveData},isDataNull:function(a){return this.super.isDataNull.call(this,a)||-1<["",null,void 0].indexOf(this._value)},getData:function(a={}){var b={context:this,options:a,data:this.super.getData.call(this,a)};return void 0===b.data&&(b.data=this.value,""===b.data&&(b.data=null)),this.inPath.length&&(b.data=Mavo.subset(this.data,this.inPath,b.data)),Mavo.hooks.run("node-getdata-end",b),b.data},sneak:function(a){return Mavo.Observer.sneak(this.observer,a)},save:function(){this.savedValue=this.value,this.unsavedChanges=!1},initEdit:function(){if(!this.editor&&this.originalEditor&&(this.editor=this.originalEditor.cloneNode(!0)),!this.editor){var c=this.config.editor;c&&"boolean"!=this.datatype||(c=Mavo.Elements.defaultConfig[this.datatype||"string"].editor),this.editor=a.create("function"===a.type(c)?c.call(this):c),this.editorValue=this.value}a.bind(this.editor,{"input change":()=>{this.value=this.editorValue},"mv-change":(b)=>{"output"===b.property&&(b.stopPropagation(),a.fire(this.editor,"input"))}});var d=this.editor.matches("textarea");d||this.editor.addEventListener("focus",()=>{this.editor.select&&this.editor.select()}),!this.popup&&this.closestCollection&&this.editor.matches(Mavo.selectors.textInput)&&this.editor.addEventListener("keydown",(a)=>{if(13==a.keyCode&&this.closestCollection.editing&&(a.shiftKey||!d)){if(this.bottomUp)return;var b=this.closestItem,c=this.closestCollection.add(void 0,b&&b.index+1);this.closestCollection.editItem(c,{immediately:!0}),copy=this.getCousin(1),copy.edit({immediately:!0}).then(()=>copy.editor.focus()),d&&a.preventDefault()}else if(8==a.keyCode&&(this.empty&&this.collection||a[Mavo.superKey])){this.closestCollection.delete(this.closestItem);var e=this.getCousin(1)||this.getCousin(-1);e&&e.edit({immediately:!0}).then(()=>e.editor.focus())}}),"placeholder"in this.editor&&(this.editor.placeholder="("+this.label+")");var e=/^mv-edit-/i;b(this.element.attributes).forEach(function(a){e.test(a.name)&&this.editor.setAttribute(a.name.replace(e,""),a.value)},this),(this.attribute||this.config.popup)&&(this.popup=new Mavo.UI.Popup(this)),this.popup||this.editor.classList.add("mv-editor"),this.initEdit=null},edit:function(b={}){return!1!==this.super.edit.call(this)&&(-1===this.element.tabIndex&&Mavo.revocably.setAttribute(this.element,"tabindex","0"),this.element.classList.add("mv-pending-edit"),this.modes||a.bind(this.element,"click.mavo:edit",(a)=>a.preventDefault()),this.preEdit=Mavo.promise((c)=>{if(b.immediately)return c();var d=["click","focus","dragover","dragenter"].map((a)=>a+".mavo:preedit").join(" ");a.bind(this.element,d,c)}).then(()=>a.unbind(this.element,".mavo:preedit")),this.config.edit?void this.config.edit.call(this):this.preEdit.then(()=>{this.element.classList.remove("mv-pending-edit"),this.sneak(()=>{this.initEdit&&this.initEdit(),this.popup&&(this.popup.prepare(),this.popup.show()),this.attribute||this.popup||(this.editor.parentNode!=this.element&&(this.editorValue=this.value,this.config.hasChildren?this.element.textContent="":c.setText(this.element,""),this.element.prepend(this.editor)),!this.collection&&(document.activeElement===this.element&&this.editor.focus(),Mavo.revocably.restoreAttribute(this.element,"tabindex")))})}))},done:function(){return!1!==this.super.done.call(this)&&void("preEdit"in this&&a.unbind(this.element,".mavo:preedit .mavo:edit"),this.sneak(()=>{return this.config.done?void this.config.done.call(this):void(this.popup?this.popup.close():!this.attribute&&this.editor&&(a.remove(this.editor),c.setValue(this.element,this.editorValue,{config:this.config,attribute:this.attribute,datatype:this.datatype,map:this.originalEditor||this.editor,node:this})))}),!this.collection&&Mavo.revocably.restoreAttribute(this.element,"tabindex"))},dataRender:function(b,c){if(b&&"object"==typeof b)if(Symbol.toPrimitive in b)b=b[Symbol.toPrimitive]("default");else{var d,e=Object.keys(b);if(1===e.length)d=e[0];else{for(let a of[this.property,"value","content"])if(a in b){d=a;break}for(let c in b)if(type=a.type(b[c]),type===this.datatype||!this.datatype&&"string"==type){d=c;break}}d&&(b=b[d],!c&&this.inPath.push(d))}b===void 0?!this.modes&&(this.value=this.closestCollection?this.default:this.templateValue):this.value=b},find:function(a,b={}){if(this.property==a&&b.exclude!==this)return this},getValue:function(){return c.getValue(this.element,{config:this.config,attribute:this.attribute,datatype:this.datatype})},lazy:{label:function(){return Mavo.Functions.readable(this.property)},emptyValue:function(){switch(this.datatype){case"boolean":return!1;case"number":return 0;}return""},editorDefaults:function(){return this.editor&&c.getConfig(this.editor)}},setValue:function(a,b={}){return this.sneak(()=>{a=a||0===a||!1===a?a:"";var d=this.datatype;return this.datatype||"number"!=typeof a&&"boolean"!=typeof a||(this.datatype=typeof a),a=c.safeCast(a,this.datatype),b.force||a!=this._value||d!=this.datatype?void(this.editor&&document.activeElement!=this.editor&&(this.editorValue=a),(this.popup||!this.editor||this.editor!==document.activeElement)&&(this.config.setValue?this.config.setValue.call(this,this.element,a):!b.dataOnly&&c.setValue(this.element,a,{config:this.config,attribute:this.attribute,datatype:this.datatype,map:this.originalEditor||this.editor,node:this})),this.empty=!a&&0!==a,this._value=a,this.updateLiveData(),!b.silent&&(this.saved&&(this.unsavedChanges=this.mavo.unsavedChanges=!0),this.dataChanged("propertychange",{value:a}))):a}),a},dataChanged:function(a="propertychange",b){return this.super.dataChanged.call(this,a,b)},live:{default:function(a){this.value==this._default&&(this.value=a)},value:function(a){return this.setValue(a)},datatype:function(b){b!==this._datatype&&("boolean"==b&&!this.attribute&&(this.attribute=Mavo.Elements.defaultConfig.boolean.attribute),a.toggleAttribute(this.element,"datatype",b,b&&"string"!==b))},empty:function(b){var c=b&&!this.modes&&!(this.attribute&&a(Mavo.selectors.property,this.element));this.element.classList.toggle("mv-empty",!!c)}},static:{all:new WeakMap,getText:function(a){var b=a.nodeType===Node.TEXT_NODE?a:a.firstChild;return b&&b.nodeType===Node.TEXT_NODE?b.nodeValue:""},setText:function(a,b){var c=a.nodeType===Node.TEXT_NODE?a:a.firstChild;c&&c.nodeType===Node.TEXT_NODE?c.nodeValue=b:a.prepend(b)},getValueAttribute:function(a,b=Mavo.Elements.search(a)){var c=a.getAttribute("mv-attribute")||b.attribute;return c&&"null"!==c&&"none"!==c||(c=null),c},safeCast:function(a,b){var d=c.cast(a,b);return null===a||void 0===a?a:"boolean"==b?"false"===a||0===a||""===a?!1:"true"===a||0<a||a:"number"==b?/^[-+]?[0-9.e]+$/i.test(a+"")?d:a:d},cast:function(a,b){return"number"===b?+a:"boolean"===b?!!a:"string"===b?a+"":a},getValue:function(a,{config:b,attribute:d,datatype:e}={}){if(b||(b=c.getConfig(a,d)),d=b.attribute,e=b.datatype,b.getValue&&d==b.attribute)return b.getValue(a);var f;return f=d in a&&c.useProperty(a,d)?a[d]:d?a.getAttribute(d):a.getAttribute("content")||c.getText(a)||null,c.safeCast(f,e)},getConfig:function(a,b,d){void 0===b&&(b=a.getAttribute("mv-attribute")||void 0),("null"==b||"none"==b)&&(b=null),isAttributeDefault=void 0===b||b==c.getValueAttribute(a),!d&&isAttributeDefault&&(d=a.getAttribute("datatype")||void 0);var e=Mavo.Elements.search(a,b,d);return void 0===e.attribute&&(e.attribute=b||null),void 0===e.datatype&&(e.datatype=d),e},setValue:function(b,d,e={}){if(1===b.nodeType&&(e.config||(e.config=c.getConfig(b,e.attribute)),e.attribute=void 0===e.attribute?e.config.attribute:e.attribute,e.datatype=void 0===e.datatype?e.config.datatype:e.datatype,e.config.setValue&&e.attribute==e.config.attribute))return e.config.setValue(b,d,e.attribute);if(e.attribute){if(e.attribute in b&&c.useProperty(b,e.attribute)&&b[e.attribute]!==d){try{var f=b[e.attribute],g=b[e.attribute]=d}catch(a){}f!=g&&e.config.changeEvents&&e.config.changeEvents.split(/\s+/).forEach((c)=>a.fire(b,c))}"boolean"==e.datatype?d!=b.hasAttribute(e.attribute)&&a.toggleAttribute(b,e.attribute,d,d):b.getAttribute(e.attribute)!=d&&b.setAttribute(e.attribute,d)}else{var h=c.format(d,e);e.node&&!e.config.hasChildren?c.setText(b,h):b.textContent=h,h!==d&&b.setAttribute&&b.setAttribute("content",d)}},useProperty:function(a,b){return!(-1<["href","src"].indexOf(b))&&"http://www.w3.org/2000/svg"!=a.namespaceURI},format:(b,d={})=>{if(d.map&&/^select$/i.test(d.map.nodeName))for(var e,f=0;e=d.map.options[f];f++)if(e.value==b)return e.textContent;if("number"===a.type(b)||"number"==d.datatype){var g=d.attribute||d.element&&d.element.matches("style, pre");if(!g)return c.formatNumber(b)}return Array.isArray(b)?b.map(c.format).join(", "):"object"===a.type(b)?Mavo.toJSON(b):b},lazy:{formatNumber:()=>{var a=new Intl.NumberFormat(Mavo.locale,{maximumFractionDigits:2});return function(b){return b===Infinity||b===-Infinity?0>b?"-\u221E":"\u221E":a.format(b)}}}}})})(Bliss,Bliss.$);
...
(function(a){var b=Mavo.Elements={};Object.defineProperties(b,{register:{value:function(c,d){if("object"==typeof arguments[0]){for(let a in arguments[0])b.register(a,arguments[0][a]);return}if(d.extend){var e=b[d.extend];d=a.extend(a.extend({},e),d)}if(-1<c.indexOf("@")){var f=c.split("@");d.selector=d.selector||f[0]||"*",void 0===d.attribute&&(d.attribute=f[1])}return d.selector=d.selector||c,d.id=c,Array.isArray(d.attribute)?d.attribute.forEach((e)=>{var f=a.extend({},d);f.attribute=e,b[`${c}@${e}`]=f}):b[c]=d,b}},search:{value:function(c,d,e){var f=b.matches(c,d,e),g=f[f.length-1];if(g)return g;var h=a.extend({},b.defaultConfig[e||"string"]);return h.attribute=void 0===d?h.attribute:d,h}},matches:{value:function(a,c,d){var e=[];selectorloop:for(var f in b){var g=b[f],h=void 0===c&&g.default||c===g.attribute;if(h&&(void 0===d||"string"===d||d===g.datatype)){var i=g.selector||f;a.matches(i)&&(!g.test||g.test(a,c,d))&&e.push(g)}}return e}},isSVG:{value:(a)=>"http://www.w3.org/2000/svg"==a.namespaceURI},defaultConfig:{value:{string:{editor:{tag:"input"}},number:{editor:{tag:"input",type:"number"}},boolean:{attribute:"content",editor:{tag:"input",type:"checkbox"}}}}}),b.register({"@hidden":{datatype:"boolean"},"@y":{test:b.isSVG,datatype:"number"},"@x":{default:!0,test:b.isSVG,datatype:"number"},media:{default:!0,selector:"img, video, audio",attribute:"src",editor:function(){var b=a.create("input",{type:"url",placeholder:"http://example.com/image.png",className:"mv-output","aria-label":"URL to image"});if(this.mavo.uploadBackend&&self.FileReader){var c,d=this.element.nodeName.toLowerCase();d="img"==d?"image":d;var e=this.element.getAttribute("mv-uploads")||d+"s",f=(a,c=a.name)=>{if(a&&0===a.type.indexOf(d+"/")){var f=URL.createObjectURL(a);this.sneak(()=>this.element.src=f),this.mavo.upload(a,e+"/"+c).then((a)=>{var c=0,d=Mavo.rr(()=>Mavo.defer(1e3+500*c).then(()=>{c++,this.element.src=a})),e=()=>{URL.revokeObjectURL(f),this.element.removeEventListener("load",g),this.element.removeEventListener("error",g)},g=()=>{this.element.src!=f&&(this.element.src=a,e())};b.value=a,this.element.addEventListener("load",g),this.element.addEventListener("error",()=>{10>=c?(this.sneak(()=>this.element.src=f),d()):(this.mavo.error(this.mavo._("cannot-load-uploaded-file")+" "+a),e())})})}},g={paste:(a)=>{var b=a.clipboardData.items[0];if("file"==b.kind&&0===b.type.indexOf(d+"/")){var c=`pasted-${d}-${Date.now()}.${b.type.slice(6)}`,e=prompt(this.mavo._("filename"),c);""===e&&(e=c),null!==e&&(f(b.getAsFile(),e),a.preventDefault())}},"drag dragstart dragend dragover dragenter dragleave drop":(a)=>{a.preventDefault(),a.stopPropagation()},"dragover dragenter":()=>{c.classList.add("mv-dragover"),this.element.classList.add("mv-dragover")},"dragleave dragend drop":()=>{c.classList.remove("mv-dragover"),this.element.classList.remove("mv-dragover")},drop:(a)=>{f(a.dataTransfer.files[0])}};return a.bind(this.element,g),c=a.create({className:"mv-upload-popup",contents:[b,{tag:"input",type:"file","aria-label":"Upload image",accept:d+"/*",events:{change:(a)=>{var b=a.target.files[0];b&&f(b)}}},{className:"mv-tip",innerHTML:"<strong>Tip:</strong> You can also drag & drop or paste!"}],events:g})}return b}},"video, audio":{attribute:["autoplay","buffered","loop"],datatype:"boolean"},details:{attribute:"open",datatype:"boolean"},"a, link":{default:!0,attribute:"href"},"input, select, button, textarea":{attribute:"disabled",datatype:"boolean"},formControl:{selector:"input",default:!0,attribute:"value",modes:"edit",changeEvents:"input change",edit:()=>{},done:()=>{},init:function(){this.editor=this.element}},select:{extend:"formControl",selector:"select",subtree:!0},textarea:{extend:"formControl",selector:"textarea",attribute:null,getValue:(a)=>a.value,setValue:(a,b)=>a.value=b},formNumber:{extend:"formControl",selector:"input[type=range], input[type=number]",datatype:"number",setValue:function(b,c){b.value=c,b.setAttribute("value",c);var d=c>b.value?"max":"min";if(!isNaN(c)&&b.value!=c&&!Mavo.data(b,"boundObserver")){var e=new Mavo.Observer(b,d,()=>{b.value=c});requestAnimationFrame(()=>{a.bind(b,"input mv-change",function c(){e.destroy(),Mavo.data(b,"boundObserver",void 0),a.unbind(b,"input mv-change",c)})}),Mavo.data(b,"boundObserver",e)}}},checkbox:{extend:"formControl",selector:"input[type=checkbox]",attribute:"checked",datatype:"boolean",changeEvents:"click"},radio:{extend:"formControl",selector:"input[type=radio]",attribute:"checked",modes:"edit",getValue:(b)=>{if(b.form)return b.form[b.name].value;var c=a(`input[type=radio][name="${b.name}"]:checked`);return c&&c.value},setValue:(b,c)=>{if(b.form)return void(b.form[b.name].value=c);var d=a(`input[type=radio][name="${b.name}"][value="${c}"]`);a.properties(d,{checked:!0})},init:function(a){this.mavo.element.addEventListener("change",(b)=>{b.target.name==a.name&&(this.value=this.getValue())})}},counter:{extend:"formControl",selector:"button, .counter",attribute:"mv-clicked",datatype:"number",init:function(a){"mv-clicked"===this.attribute&&(a.setAttribute("mv-clicked","0"),a.addEventListener("click",()=>{let b=+a.getAttribute("mv-clicked")||0;this.value=++b}))}},"meter, progress":{default:!0,attribute:"value",datatype:"number",edit:function(){var b=Math.min,c=Math.max,d=+this.element.getAttribute("min")||0,e=+this.element.getAttribute("max")||1,f=e-d,g=+this.element.getAttribute("mv-edit-step")||(1<f?1:f/100);a.bind(this.element,"mousemove.mavo:edit",(a)=>{var h=this.element.getBoundingClientRect().left,i=c(0,(a.clientX-h)/this.element.offsetWidth),j=d+f*i,k=j%g;j+=k>g/2?g-k:-k,j=c(d,b(j,e)),this.sneak(()=>this.element.setAttribute("value",j))}),a.bind(this.element,"mouseleave.mavo:edit",()=>{this.sneak(()=>this.element.setAttribute("value",this.value))}),a.bind(this.element,"click.mavo:edit",()=>{this.value=this.getValue()}),a.bind(this.element,"keydown.mavo:edit",(a)=>{if(a.target==this.element&&(37==a.keyCode||39==a.keyCode)){var f=g*(39==a.keyCode?1:-1)*(a.shiftKey?10:1),h=this.value+f;h=c(d,b(h,e)),this.element.setAttribute("value",h),a.preventDefault()}})},done:function(){a.unbind(this.element,".mavo:edit")}},meta:{default:!0,attribute:"content"},block:{default:!0,selector:"p, div, dt, dd, h1, h2, h3, h4, h5, h6, article, section, address",editor:function(){var b=getComputedStyle(this.element),c=b.display,d=0===c.indexOf("inline")?"input":"textarea",e=a.create(d);if("textarea"==d){var f=this.element.offsetWidth;f&&(e.width=f),e.style.whiteSpace={normal:"pre-wrap",nowrap:"pre"}[b.whiteSpace]||"inherit"}return e},setEditorValue:function(a){this.datatype&&"string"!=this.datatype&&(a+="");var b=getComputedStyle(this.element);return a=a||"",-1<["normal","nowrap"].indexOf(b.whiteSpace)&&(a=a.replace(/\r?\n/g," ")),-1<["normal","nowrap","pre-line"].indexOf(b.whiteSpace)&&(a=a.replace(/^[ \t]+|[ \t]+$/gm,"").replace(/[ \t]+/g," ")),this.editor.value=a,!0}},time:{attribute:"datetime",default:!0,init:function(){if(!this.fromTemplate("dateType")){var a=Mavo.DOMExpression.search(this.element,null),b=this.element.getAttribute("datetime")||"YYYY-MM-DD";for(var c in this.config.dateTypes)if(this.config.dateTypes[c].test(b))break;this.dateType=c,a||(this.element.textContent=this.config.defaultFormats[this.dateType](this.property),this.mavo.expressions.extract(this.element,null))}},dateTypes:{month:/^[Y\d]{4}-[M\d]{2}$/i,time:/^[H\d]{2}:[M\d]{2}/i,"datetime-local":/^[Y\d]{4}-[M\d]{2}-[D\d]{2} [H\d]{2}:[Mi\d]{2}/i,date:/^[Y\d]{4}-[M\d]{2}-[D\d]{2}$/i},defaultFormats:{date:(a)=>`[day(${a})] [month(${a}).shortname] [year(${a})]`,month:(a)=>`[month(${a}).name] [year(${a})]`,time:(a)=>`[digits(2, hour(${a}))]:[digits(2, minute(${a}))]`,"datetime-local":function(a){return this.date(a)+" "+this.time(a)}},editor:function(){return{tag:"input",type:this.dateType}}},"circle@r":{default:!0,datatype:"number"},circle:{attribute:["cx","cy"],datatype:"number"},text:{default:!0,popup:!0},".mv-toggle":{default:!0,attribute:"aria-checked",datatype:"boolean",edit:function(){Mavo.revocably.setAttribute(this.element,"role","checkbox"),a.bind(this.element,"click.mavo:edit keyup.mavo:edit keydown.mavo:edit",(a)=>{("click"==a.type||" "==a.key||"Enter"==a.key)&&("keydown"!=a.type&&(this.value=!this.value),a.preventDefault(),a.stopPropagation())})},done:function(){Mavo.revocably.restoreAttribute(this.element,"role"),a.unbind(this.element,".mavo:edit")}}})})(Bliss,Bliss.$);
(function(a,b){Mavo.attributes.push("mv-multiple","mv-order","mv-accepts");var d=Mavo.Collection=a.Class({extends:Mavo.Node,nodeType:"Collection",constructor:function(){if(this.templateElement=this.element,this.children=[],this.liveData=this.createLiveData([]),this.parent.liveData[this.property]=this.liveData[Mavo.toProxy],this.marker=document.createComment("mv-marker"),Mavo.data(this.marker,"collection",this),a.after(this.marker,this.templateElement),this.fromTemplate("templateElement","accepts","optional","like","likeNode")||(this.like=this.templateElement.getAttribute("mv-like"),this.like&&(this.likeNode=this.resolve(this.like,{exclude:this}),this.likeNode?this.likeNode=this.likeNode.template||this.likeNode:this.like=null),this.accepts=this.templateElement.getAttribute("mv-accepts"),this.accepts=this.accepts&&this.accepts.split(/\s+/)||[],this.optional=!!this.like||this.templateElement.hasAttribute("mv-optional"),this.templateElement=this.templateElement.cloneNode(!0)),this.likeNode){this.itemTemplate=this.likeNode.itemTemplate||this.likeNode;var b=this.likeNode.templateElement||a.value(this.likeNode.collection,"templateElement")||this.likeNode.element;this.templateElement=b.cloneNode(!0),this.templateElement.setAttribute("property",this.property),this.accepts.length||(this.accepts=this.likeNode.accepts||this.accepts),this.properties=this.likeNode.properties}else if(!this.optional||!this.template){var c=this.createItem(this.element);this.add(c,void 0,{silent:!0}),this.optional&&this.delete(c,{silent:!0})}this.optional&&this.element.remove(),this.postInit(),Mavo.hooks.run("collection-init-end",this)},get length(){return this.children.length},getData:function(a={}){for(var b,c={context:this,options:a,data:this.liveData},d=0,e=0;item=this.children[d];d++)b=item.getData(c.options),null!==Mavo.value(b)&&(c.data[e]=b,e++);return c.data.length=e,c.data=Mavo.subset(this.data,this.inPath,c.data),Mavo.hooks.run("node-getdata-end",c),c.data},createItem:function(a){a||(a=this.templateElement.cloneNode(!0));var b=this.itemTemplate||(this.template?this.template.itemTemplate:null),c=Mavo.Node.create(a,this.mavo,{collection:this,template:b,property:this.property,type:this.type});return this.itemTemplate||(this.itemTemplate=b||c),c},add:function(b,c,d={}){b=b instanceof Node?Mavo.Node.get(b)||this.createItem(b):b||this.createItem(),b.collection!=this&&this.adopt(b);var e=this.children[c]?this.children[c].element:this.marker;a[this.bottomUp?"after":"before"](b.element,e),void 0===c&&(c=this.bottomUp?0:this.length);var f={context:this,item:b};return f.previousIndex=b.index,f.changed=this.splice({remove:f.item},{index:c,add:f.item}),f.item.itembar&&f.item.itembar.reposition(),this.mavo.expressions.active&&!d.silent&&requestAnimationFrame(()=>{f.changed.forEach((a)=>{a.dataChanged(a==f.item&&void 0===f.previousIndex?"add":"move"),a.unsavedChanges=!0}),this.unsavedChanges=this.mavo.unsavedChanges=!0,this.mavo.expressions.update(f.item)}),Mavo.hooks.run("collection-add-end",f),f.item},splice:function(...a){a.forEach((a)=>{void 0===a.index&&a.remove&&isNaN(a.remove)&&(a.index=this.children.indexOf(a.remove),a.remove=1)}),a.sort((c,a)=>a.index-c.index),a.forEach((a)=>{-1<a.index&&(a.remove||a.add)&&(a.remove=a.remove||0,a.add=Mavo.toArray(a.add),this.children.splice(a.index,+a.remove,...a.add))});var b=[];for(let c,d=0;d<this.length;d++)c=this.children[d],c&&c.index!==d&&(c.index=d,b.push(c));return this.updateLiveData(),b},adopt:function(a){a.collection&&(a.collection.splice({remove:a}),a.collection.dataChanged("delete")),a.collection=this,this.walk((b)=>{b.closestCollection===a.collection&&(b.closestCollection=this),a.mavo!=this.mavo&&(b.mavo=this.mavo),b.path=b.getPath()}),a.template&&(Mavo.delete(a.template.copies,a),a.template=this.itemTemplate)},delete:function(b,{silent:c,undoable:d=!c,transition:e=!c}={}){if(b.element.classList.remove("mv-highlight"),this.splice({remove:b}),c)return a.remove(b.element),d||b.destroy(),Promise.resolve(b);if(e)var f=a.transition(b.element,{opacity:0}).then(()=>{b.element.style.opacity=""});else var f=Promise.resolve();return f.then(()=>{return a.remove(b.element),this.unsavedChanges=b.unsavedChanges=this.mavo.unsavedChanges=!0,b.dataChanged("delete",{index:b.index}),d&&this.mavo.setDeleted(b),b})},move:function(a,b){var c=a.index+b+(0<b);c=Mavo.wrap(c,this.children.length+1),this.add(a,c),a instanceof Mavo.Primitive&&a.itembar&&a.itembar.reposition()},editItem:function(a,b={}){var c=b.immediately?Promise.resolve():Mavo.inView.when(a.element);return c.then(()=>{return a.itembar||(a.itembar=new Mavo.UI.Itembar(a)),a.itembar.add(),a.edit(b)})},edit:function(b={}){if(!1===this.super.edit.call(this))return!1;if(!this.addButton.parentNode){var c=this.element.tagName.toLowerCase(),e=Mavo.selectors.container[c],f=e?this.marker.parentNode.closest(e):this.marker;a[this.bottomUp?"before":"after"](this.addButton,f)}return d.dragula.then(()=>{this.getDragula()}),Promise.all(this.children.map((a)=>this.editItem(a,b)))},done:function(){return!1!==this.super.done.call(this)&&void(this.addButton.parentNode&&this.addButton.remove(),this.propagate((a)=>{a.itembar&&a.itembar.remove()}))},dataChanged:function(a,b={}){return b.element=b.element||this.marker,this.super.dataChanged.call(this,a,b)},save:function(){this.children.forEach((a)=>a.unsavedChanges=!1)},propagated:["save"],updateLiveData:function(){this.liveData.length=0;for(var a=0;a<this.children.length;a++)this.liveData[a]=this.children[a].getLiveData()},dataRender:function(b){if(void 0!==b){b=null===b?[]:Mavo.toArray(b).filter((a)=>null!==a);for(var c,d=0;d<this.children.length;d++)c=this.children[d],d<b.length?c.render(b[d]):(this.delete(c,{silent:!0}),d--);if(b.length>d){for(var c,e=document.createDocumentFragment(),f=d;f<b.length;f++){c=this.createItem(),c.render(b[f]),this.children.push(c),c.index=f,e.appendChild(c.element);var g={context:this,item:c};Mavo.hooks.run("collection-add-end",g)}this.bottomUp?a.after(e,0<d?this.children[d-1].element:this.marker):a.before(e,this.marker)}if(this.updateLiveData(),b.length>d)for(var f=d;f<this.children.length;f++)this.children[f].dataChanged("add"),this.mavo.expressions.active&&requestAnimationFrame(()=>this.mavo.expressions.update(this.children[f]))}},find:function(a,b={}){if(b.exclude!==this){var c=this.children.filter((a)=>!a.hidden);if(this.property==a)return b.collections?this:c;if(this.properties.has(a)){var d=c.map((c)=>c.find(a,b));return Mavo.flatten(d)}}},isCompatible:function(a){return a&&this.itemTemplate.nodeType==a.itemTemplate.nodeType&&(a===this||a.template==this||this.template==a||this.template&&this.template==a.template||-1<this.accepts.indexOf(a.property))},getDragula:function(){if(this.dragula)return this.dragula;if(this.template)return Mavo.pushUnique(this.template.getDragula().containers,this.marker.parentNode),this.dragula=this.template.dragula||this.template.getDragula();this;return this.dragula=dragula({containers:[this.marker.parentNode],isContainer:(a)=>{return!!this.accepts.length&&-1<Mavo.flatten(this.accepts.map((a)=>this.mavo.root.find(a,{collections:!0}))).filter((a)=>a&&a instanceof d).map((a)=>a.marker.parentNode).indexOf(a)},moves:(a,b,c)=>{return c.classList.contains("mv-drag-handle")&&c.closest(Mavo.selectors.multiple)==a},accepts:function(a,b,c,e){if(a.contains(b))return!1;var f=e?e.previousElementSibling:b.lastElementChild,g=d.get(f)||d.get(e);if(!g)return!1;var h=Mavo.Node.get(a);return h&&h.collection.isCompatible(g)}}),this.dragula.on("drop",(a)=>{var b=Mavo.Node.get(a),c=b&&b.index,e=a.nextElementSibling,f=a.previousElementSibling,g=d.get(f)||d.get(e),h=Mavo.Node.get(f)||Mavo.Node.get(e);if(h&&h.collection!=g&&(h=null),b.collection.isCompatible(g)){var i=h?h.index+(h.element===f):g.length;g.add(b,i)}else return this.dragula.cancel(!0)}),d.dragulas.push(this.dragula),this.dragula},getClosestCollection:function(){return this},lazy:{bottomUp:function(){var a=this.templateElement.getAttribute("mv-order");return null===a?!!this.addButton.parentNode&&!!(this.addButton.compareDocumentPosition(this.marker)&Node.DOCUMENT_POSITION_FOLLOWING):/^desc\b/i.test(a)},addButton:function(){var c=`button.mv-add-${this.property}`,d=this.marker.parentNode.closest(Mavo.selectors.item);if(d)var e=b(c,d).filter((a)=>{return!this.templateElement.contains(a)&&!Mavo.data(a,"collection")})[0];return e||(e=a.create("button",{className:"mv-add",textContent:this.mavo._("add-item",this)})),e.classList.add("mv-ui","mv-add"),Mavo.data(e,"collection",this),this.property&&e.classList.add(`mv-add-${this.property}`),e.addEventListener("click",(a)=>{a.preventDefault(),this.editItem(this.add())}),e},liveData:function(){return this.createLiveData([])}},static:{dragulas:[],get:(a)=>{var b=Mavo.data(a,"collection");if(b)return b;var c=Mavo.Node.get(a);return c&&c.collection||null},delete:(a,b={})=>{var c=new Mavo.BucketMap({arrays:!0}),d=a.filter((a)=>!!a.collection).map((a)=>a.collection.delete(a,Object.assign({},b,{undoable:!1})).then((a)=>c.set(a.mavo,a)));b.silent||!1===b.undoable||Promise.all(d).then(()=>{c.forEach((a,b)=>{b.setDeleted(...a)})})},lazy:{dragula:()=>a.include(self.dragula,"https://cdnjs.cloudflare.com/ajax/libs/dragula/3.7.2/dragula.min.js")}}})})(Bliss,Bliss.$);
(function(a){Mavo.ImplicitCollection=a.Class({extends:Mavo.Node,nodeType:"ImplicitCollection",constructor:function(a){this.children=[],this.liveData=this.createLiveData([]),this.parent.liveData[this.property]=this.liveData[Mavo.toProxy],this.add(a),this.postInit(),Mavo.hooks.run("implicit-collection-init-end",this)},get length(){return this.children.length},updateLiveData:function(){this.liveData.length=0;for(var a,b=0;b<this.children.length;b++)if(a=this.children[b],delete a.liveData[0],!a.isDataNull({live:!0})){let b=a.getLiveData();this.liveData.push(b)}var c=1===this.liveData.length?this.liveData[0]:this.liveData;this.parent.liveData[this.property]=c[Mavo.toProxy]},getData:function(a={}){var b={context:this,options:a,data:[]};if(this.children.forEach((c)=>{c.isDataNull()||b.data.push(c.getData(a))}),this.data){var c=Mavo.toArray(Mavo.subset(this.data,this.inPath));c.length>b.data.length&&(b.data=b.data.concat(c.slice(b.data.length)))}return Array.isArray(b.data)&&1>=b.data.length&&(b.data=1===b.data.length?b.data[0]:null),b.data=Mavo.subset(this.data,this.inPath,b.data),Mavo.hooks.run("node-getdata-end",b),b.data},add:function(a){var b=Mavo.Node.create(a,this.mavo,{collection:this,template:this.template&&this.template.children[this.length]||null,property:this.property,type:this.type});return b.index=this.length,this.children.push(b),b},edit:function(a={}){return!1!==this.super.edit.call(this)&&Promise.all(this.children.map((b)=>b.edit(a)))},propagated:["save"],dataRender:function(a){a!==void 0&&(a=null===a?[]:Mavo.toArray(a).filter((a)=>null!==a),this.children.forEach((b,c)=>b.render(a&&a[c]))),this.updateLiveData()},find:function(a,b={}){if(b.exclude!==this){var c=this.children.filter((a)=>!a.deleted&&!a.hidden);if(this.property==a)return b.collections?this:c;if(this.properties.has(a)){var d=c.map((c)=>c.find(a,b));return Mavo.flatten(d)}}},lazy:{liveData:function(){return this.createLiveData([])}}})})(Bliss,Bliss.$);
...
(function(a){var b=Mavo.Expression=a.Class({constructor:function(a){this.expression=a},eval:function(a,b){Mavo.hooks.run("expression-eval-beforeeval",this);try{return this.function||(this.function=Mavo.Script.compile(this.expression,b)),this.function(a)}catch(a){return console.info("%cExpression error!","color: red; font-weight: bold",`${a.message} in expression ${this.expression}`,`
Not an expression? Use mv-expressions="none" to disable expressions on an element and its descendants.`),Mavo.hooks.run("expression-eval-error",{context:this,exception:a}),a}},toString(){return this.expression},changedBy:function(a){return b.changedBy(this.identifiers,a)},live:{expression:function(a){this.function=null,this.identifiers=a.match(/[$a-z][$\w]*/ig)||[]}}});b.Syntax=a.Class({constructor:function(a,b){this.start=a,this.end=b,this.regex=RegExp(`${Mavo.escapeRegExp(a)}([\\S\\s]+?)${Mavo.escapeRegExp(b)}`,"gi")},test:function(a){return this.regex.lastIndex=0,this.regex.test(a)},tokenize:function(a){var b,c=[],d=0;for(this.regex.lastIndex=0;null!==(b=this.regex.exec(a));)b.index>d&&c.push(a.substring(d,b.index)),d=this.regex.lastIndex,c.push(new Mavo.Expression(b[1]));return d<a.length&&c.push(a.substring(d)),c},static:{create:function(a){if(a){var c=a.getAttribute("mv-expressions");if(c)return c=c.trim(),/\s/.test(c)?new b.Syntax(...c.split(/\s+/)):b.Syntax.ESCAPE}},ESCAPE:-1}}),b.Syntax.default=new b.Syntax("[","]")})(Bliss,Bliss.$);
(function(a){var b=Mavo.DOMExpression=a.Class({constructor:function(c={}){this.mavo=c.mavo,this.template=c.template&&c.template.template||c.template;for(let a of["item","path","syntax","fallback","attribute","originalAttribute","expression","parsed","identifiers"])this[a]=void 0===c[a]&&this.template?this.template[a]:c[a];if(this.node=c.node,this.node||(this.node=Mavo.elementPath(this.item.element,this.path)),this.element=this.node,this.attribute=this.attribute||null,Mavo.hooks.run("domexpression-init-start",this),"mv-value"==this.attribute){this.originalAttribute="mv-value",this.attribute=Mavo.Primitive.getValueAttribute(this.element),this.fallback=this.fallback||Mavo.Primitive.getValue(this.element,{attribute:this.attribute});var d=this.element.getAttribute("mv-value");this.element.removeAttribute("mv-value"),this.parsed=[new Mavo.Expression(d)],this.expression=this.syntax.start+d+this.syntax.end}if(3===this.node.nodeType&&this.element===this.node&&(this.element=this.node.parentNode,(!this.node.parentNode.children.length||this.attribute)&&(this.node=this.element,this.element.normalize())),!this.expression){if(this.attribute){var e=Element.prototype.getAttribute.call(this.node,this.attribute);this.expression=(e||"").trim()}else{if(this.node.normalize(),this.node.firstChild&&1===this.node.childNodes.length&&3===this.node.firstChild.nodeType){var f=this.node.firstChild.textContent.match(/^\s*|\s*$/g);f[1]&&(this.node.firstChild.splitText(this.node.firstChild.textContent.length-f[1].length),a.after(this.node.lastChild,this.node)),f[0]&&(this.node.firstChild.splitText(f[0].length),this.node.parentNode.insertBefore(this.node.firstChild,this.node))}this.expression=this.node.textContent}this.parsed=c.template?c.template.parsed:this.syntax.tokenize(this.expression)}this.oldValue=this.value=this.parsed.map((a)=>a instanceof Mavo.Expression?a.expression:a),this.identifiers=this.identifiers||Mavo.flatten(this.parsed.map((a)=>a.identifiers||[])),b.special.add(this),this.mavo.treeBuilt.then(()=>{this.template||this.item||(this.item=Mavo.Node.getClosestItem(this.element)),"mv-value"==this.originalAttribute&&this.mavoNode&&this.mavoNode==this.item.collection&&Mavo.delete(this.item.expressions,this),this.mavo.expressions.register(this),Mavo.hooks.run("domexpression-init-treebuilt",this)}),Mavo.hooks.run("domexpression-init-end",this),b.elements.set(this.element,[...(b.elements.get(this.element)||[]),this])},destroy:function(){b.special.delete(this),this.mavo.expressions.unregister(this)},changedBy:function(a){return"mv-value"==this.originalAttribute&&this.mavoNode&&!(this.mavoNode instanceof Mavo.Primitive)?!a||!this.mavoNode.contains(a.node):Mavo.Expression.changedBy(this.identifiers,a)},update:function(a=this.data){var b={context:this},c=b;this.data=a,Mavo.hooks.run("domexpression-update-start",b),this.oldValue=this.value;var d=!1;b.value=this.value=this.parsed.map((b)=>{if(b instanceof Mavo.Expression){var e={context:this,expr:b,parentEnv:c};return Mavo.hooks.run("domexpression-update-beforeeval",e),e.value=Mavo.value(e.expr.eval(a)),Mavo.hooks.run("domexpression-update-aftereval",e),d=!0,e.value instanceof Error?void 0===this.fallback?this.syntax.start+e.expr.expression+this.syntax.end:this.fallback:void 0===e.value||null===e.value?"":e.value}return b});d&&(b.value=1===b.value.length?b.value[0]:b.value.map((a)=>Mavo.Primitive.format(a,{attribute:this.attribute,element:this.element})).join(""),this.output(b.value),Mavo.hooks.run("domexpression-update-end",b))},output:function(a){this.primitive?this.primitive.value=a:this.mavoNode?this.mavoNode.render(a):Mavo.Primitive.setValue(this.node,a,{attribute:this.attribute})},live:{item:function(a){a&&this._item!=a&&(this._item&&Mavo.delete(this._item.expressions,this),a.expressions=a.expressions||[],a.expressions.push(this))}},static:{elements:new WeakMap,search:function(a,c){if(null===a)return a;var d=b.elements.get(a)||[];return 1<arguments.length?d.length?d.filter((a)=>a.attribute===c)[0]||null:null:d},special:{add:function(a,b){if(b){var c=this.vars[b];c&&-1<a.identifiers.indexOf(b)&&(c.all=c.all||new Set,c.all.add(a),1===c.all.size?c.observe():!c.all.size&&c.unobserve())}else for(var b in this.vars)this.add(a,b)},delete:function(a,b){if(b){var c=this.vars[b];c.all=c.all||new Set,c.all.delete(a),c.all.size||c.unobserve()}else for(var b in this.vars)this.delete(a,b)},update:function(){this.update&&this.update(...arguments),this.all.forEach((a)=>a.update())},event:function(a,{type:c,update:d,target:e=document}={}){this.vars[a]={observe:function(){this.callback=this.callback||b.special.update.bind(this),e.addEventListener(c,this.callback)},unobserve:function(){e.removeEventListener(c,this.callback)}},d&&(this.vars[a].update=function(b){Mavo.Functions[a]=d(b)})},vars:{$now:{observe:function(){var a=()=>{b.special.update.call(this),this.timer=requestAnimationFrame(a)};this.timer=requestAnimationFrame(a)},unobserve:function(){cancelAnimationFrame(this.timer)}}}}}});b.special.event("$mouse",{type:"mousemove",update:function(a){return{x:a.clientX,y:a.clientY}}}),b.special.event("$hash",{type:"hashchange",target:window})})(Bliss,Bliss.$);
(function(a,b){Mavo.attributes.push("mv-expressions");var c=Mavo.Expressions=a.Class({constructor:function(a){this.mavo=a,this.active=!0,this.expressions=[],this.identifiers={};var b=Mavo.Expression.Syntax.create(this.mavo.element.closest("[mv-expressions]"))||Mavo.Expression.Syntax.default;this.traverse(this.mavo.element,void 0,b),this.scheduled={},this.mavo.treeBuilt.then(()=>{this.expressions=[],this.update()})},register:function(a){var b=this.identifiers;a.identifiers.forEach((c)=>{b[c]instanceof Set||(b[c]=new Set),b[c].add(a),Mavo.all[c]instanceof Mavo&&Mavo.all[c]!==this.mavo&&Mavo.all[c].expressions.register(a)})},unregister:function(a){var b=this.identifiers;a.identifiers.forEach((a)=>{b[a]&&b[a].delete(this),a in Mavo.all&&Mavo.all[a].expressions.unregister(domexpresssion)})},updateThrottled:function(a){if(this.active){var b=this.scheduled[a.action]=this.scheduled[a.action]||new Set;a.node.template?!b.has(a.node.template)&&(setTimeout(()=>{b.delete(a.node.template),this.update(a)},c.THROTTLE),b.add(a.node.template)):requestAnimationFrame(()=>this.update(a))}},update:function(a){if(this.active){var b,c;if(a instanceof Mavo.Node)c=a;else if(a instanceof Element)b=a.closest(Mavo.selectors.item),c=Mavo.Node.get(b);else{if(a){var d={updated:new Set};if(this.updateByIdThrottled(a.property,a,d),"propertychange"==a.action)a.node&&a.node.path&&this.updateByIdThrottled(a.node.path,a,d);else{this.updateById(Object.keys(Mavo.Node.special),a,d);var e=a.node.collection||a.node;this.updateById(e.properties,a,d)}return}c=this.mavo.root}c.walk((a)=>{if(a.expressions&&a.expressions.length){var b=a.getLiveData();a.expressions.forEach((a)=>a.update(b))}})}},updateByIdThrottled:function(a,b,d){if(a)if(a.forEach)a.forEach((a)=>this.updateByIdThrottled(a,b,d));else{var e=this.scheduledIds=this.scheduledIds||new Set;e.has(a)||(setTimeout(()=>{e.delete(a),this.updateById(a,b,d)},c.THROTTLE),e.add(a))}},updateById:function(a,b,c){var d=this.identifiers[a];d&&d.forEach((a)=>{if(!("mv-value"==a.originalAttribute&&a.mavoNode&&!(a.mavoNode instanceof Mavo.Primitive)&&a.mavoNode.contains(b.node))&&!c.updated.has(a)){var d=a.item.getLiveData();a.update(d)}})},extract:function(a,b,d,e=Mavo.Expression.Syntax.default){b&&-1<c.skip.indexOf(b.name)||(b&&-1<c.directives.indexOf(b.name)||e!==Mavo.Expression.Syntax.ESCAPE&&e.test(b?b.value:a.textContent))&&(d===void 0&&(d=Mavo.elementPath(a.closest(Mavo.selectors.item),a)),this.expressions.push(new Mavo.DOMExpression({node:a,syntax:e,path:d,attribute:b&&b.name,mavo:this.mavo})))},traverse:function(a,c=[],d){if(8!==a.nodeType)if(3===a.nodeType)this.extract(a,null,c,d);else{a.normalize(),d=Mavo.Expression.Syntax.create(a)||d,Mavo.is("item",a)&&(c=[]),b(a.attributes).forEach((b)=>this.extract(a,b,c,d));var e=-1,f=0;a.matches("script:not([mv-expressions])")||b(a.childNodes).forEach((a)=>{if(1==a.nodeType?(f=0,e++):f++,1==a.nodeType||3==a.nodeType){var b=0<f?`${e}.${f}`:e;this.traverse(a,[...(c||[]),b],d)}})}},static:{directives:["mv-value"],skip:["mv-expressions","mv-action"],THROTTLE:50,directive:function(a,b){c.directives.push(a),Mavo.attributes.push(a),Mavo.Plugins.register(a,b)}}})})(Bliss,Bliss.$);
(function(a,b){Mavo.Expressions.directive("mv-if",{extend:{Primitive:{live:{hidden:function(a){this._hidden!==a&&(this._hidden=a,this.updateParentLiveData(a?null:void 0),this.dataChanged())}}},DOMExpression:{lazy:{childProperties:function(){var a=b(Mavo.selectors.property,this.element).filter((a)=>a.closest("[mv-if]")==this.element).map((a)=>Mavo.Node.get(a));return this.element.addEventListener("mv-change",(a)=>{requestAnimationFrame(()=>{this.element.parentNode||this.item.element.dispatchEvent(a)})}),a}}}},hooks:{"domexpression-init-start":function(){"mv-if"!=this.attribute||(this.expression=this.element.getAttribute("mv-if"),this.parsed=[new Mavo.Expression(this.expression)],this.expression=this.syntax.start+this.expression+this.syntax.end,this.parentIf=this.element.parentNode&&Mavo.DOMExpression.search(this.element.parentNode.closest("[mv-if]"),"mv-if"),this.parentIf&&(this.parentIf.childIfs=(this.parentIf.childIfs||new Set).add(this)))},"domexpression-update-end":function(){if("mv-if"==this.attribute){var a=this.value[0],b=this.oldValue[0];this.item.mavo.treeBuilt.then(()=>{if(this.parentIf){var c=this.parentIf.value[0];this.value[0]=a=a&&c}!1!==c&&(a?Mavo.revocably.add(this.element):this.element.parentNode&&Mavo.revocably.remove(this.element,"mv-if")),a!==b&&(this.childProperties&&this.childProperties.forEach((b)=>b.hidden=!a),this.childIfs&&this.childIfs.forEach((a)=>a.update()))})}},"unit-isdatanull":function(a){a.result=a.result||this.hidden&&a.options.live}}})})(Bliss,Bliss.$);
(function(a,b){function c(a=""){return a=b(a),a||0===a?a+"":""}function d(a){return a=Mavo.value(a),null===a||!1===a||""===a}function e(a){return!b(a)}var f=Math.min,g=Mavo.Functions={operators:{"=":"eq"},get:function(a,c,d={}){c=d.property=b(c);var e=Mavo.getCanonicalProperty(a,c);if(void 0!==e){d.property=e;var f=a[e];return"function"==typeof f&&0!==f.name.indexOf("bound")?f.bind(a):f}if(Array.isArray(a)&&c&&isNaN(c)){var h=c.indexOf("=");return-1<h?(d.query={property:c.slice(0,h),value:c.slice(h+1)},d.property=[],f=a.filter((a,b)=>{var c=g.get(a,d.query.property)==d.query.value;return c&&d.property.push(b),c}),"id"==d.query.property&&(d.property=d.property[0],f=f[0]),void 0===f?d.property=a.length:0===f.length&&(d.property=[a.length]),f):a.map((a)=>g.get(a,c))}return null},call:function(a,b,c){if(a){if("function"!=typeof a){var d=Mavo.toArray(a)[0][Mavo.toNode];d&&(a=g._Trap[d.property])}return"function"==typeof a?a.apply(c,b):void 0}},url:(a,b=location)=>{if(void 0===a)return location.href;if(a){a=c(a).replace(/[^\w-:]/g);var d=b.search.match(RegExp(`[?&]${a}(?:=(.+?))?(?=$|&)`))||b.pathname.match(RegExp(`(?:^|\\/)${a}\\/([^\\/]*)`))}return null!==d&&a?decodeURIComponent(d[1])||"":null},first:(a,b)=>{return void 0===b&&(b=a,a=void 0),Array.isArray(b)?a&&1<a?b.slice(0,a):b[0]:a&&1<a?[b]:b},last:(a,b)=>{return void 0===b&&(b=a,a=void 0),Array.isArray(b)?a&&1<a?b.slice(-a):b[b.length-1]:a&&1<a?[b]:b},unique:function(a){return Array.isArray(a)?[...new Set(a.map(b))]:a},intersects:function(a,c){if(a&&c){var d=new Set(c.map?c.map(b):c);return a=a.map?a.map(b):[...a],!a.every((a)=>!d.has(a))}},sum:function(a){return h.numbers(a,arguments).reduce((a,b)=>{return+a+(+b||0)},0)},average:function(a){return a=h.numbers(a,arguments),a.length&&g.sum(a)/a.length},min:function(a){return f(...h.numbers(a,arguments))},max:function(a){return Math.max(...h.numbers(a,arguments))},count:function(a){return Mavo.toArray(a).filter((b)=>!d(b)).length},reverse:function(a){return Mavo.toArray(a).slice().reverse()},round:function(a,b){return e(a)||e(b)||!isFinite(a)?Math.round(a):+a.toLocaleString("en-US",{useGrouping:!1,maximumFractionDigits:b})},ordinal:function(a){if(d(a))return"";if(10>b||20<b)var b=["th","st","nd","th"][a%10];return b||"th"},digits:(a,b,c)=>{if(void 0===c&&(c=b,b=void 0),isNaN(c))return null;var d=(c+"").split(".");return d[0]=d[0].slice(-a),void 0!==b&&d[1]&&(d[1]=d[1].slice(0,b)),c=+d.join("."),c.toLocaleString("en",{useGrouping:!1,minimumIntegerDigits:a,minimumFractionDigits:b,maximumFractionDigits:b||20})},iff:function(a,d=a,e=null){return Array.isArray(a)?a.map((a,c)=>{var g=b(a)?d:e;return Array.isArray(g)?g[f(c,g.length-1)]:g}):b(a)?d:e},group:(...a)=>Object.assign(...a),list:(...a)=>a,random:function(a=0,b=100,c=1){1==arguments.length&&(b=a,a=0);var d=Math.random(),e=(b-a)/c;return Math.floor(d*(e+1))*c+a},shuffle:(a)=>{return Array.isArray(a)?a.sort(()=>Math.random()-0.5):a},replace:function(a,b,c="",d=1){if(Array.isArray(a))return a.map((a)=>g.replace(a,b,c));for(var e,f=RegExp(Mavo.escapeRegExp(b),"g"),h=a,i=0;h!=e&&i++<d;)e=h,h=h.replace(f,c);return h},len:(a)=>c(a).length,search:(a,b)=>a&&b?c(a).toLowerCase().indexOf((b+"").toLowerCase()):-1,starts:(a,b)=>0===g.search(c(a),c(b)),ends:function(a,b){[a,b]=[c(a),c(b)];var d=g.search(a,b);return-1<d&&d===a.length-b.length},join:function(a,b){return Mavo.toArray(a).filter((b)=>!d(b)).join(c(b))},idify:function(a){return c(a).normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^\w\s-]/g,"").trim().replace(/\s+/g,"-").toLowerCase()},readable:function(a){return c(a).replace(/([a-z])([A-Z])(?=[a-z])/g,(a,b,c)=>b+" "+c.toLowerCase()).replace(/([a-z0-9])[_\/-](?=[a-z0-9])/g,"$1 ").replace(/^[a-z]/,(a)=>a.toUpperCase())},uppercase:(a)=>c(a).toUpperCase(),lowercase:(a)=>c(a).toLowerCase(),from:(a,b)=>g.between(a,b),fromlast:(a,b)=>g.between(a,b,"",!0),to:(a,b)=>g.between(a,"",b),tofirst:(a,b)=>g.between(a,"",b,!0),between:(a,b,d,e)=>{[a,b,d]=[c(a),c(b),c(d)];var f=b?a[e?"lastIndexOf":"indexOf"](b):-1,g=a[e?"indexOf":"lastIndexOf"](d);return b&&-1===f||-1===g?"":a.slice(f+1,-1!==g&&d?g:a.length)},filename:(a)=>Mavo.match(new URL(c(a),Mavo.base).pathname,/[^/]+?$/),json:(a)=>Mavo.safeToJSON(a),split:(a,b=/\s+/)=>{return Mavo.Script.binaryOperation(a,b,{scalar:(a,b)=>{return a=c(a),a.split(b)}})},log:(...a)=>{return console.log(...a.map(b)),a[0]},$mouse:{x:0,y:0},get $hash(){return location.hash.slice(1)},util:{numbers:function(a,c){return a=Array.isArray(a)?a:c?$$(c):[a],a.filter((a)=>!isNaN(a)&&""!==b(a)&&null!==b(a)).map((a)=>+a)}}},h=g.util;g._Trap=self.Proxy?new Proxy(g,{get:(a,b)=>{var c;if("symbol"!=typeof b){var d=b.toLowerCase&&b.toLowerCase()||b;return Mavo.Actions.running&&d in Mavo.Actions.Functions?Mavo.Actions.Functions[d]:(c=a[d]||Math[b]||Math[d],void 0===c?self&&self.hasOwnProperty(b)?self[b]:b:("function"==typeof c&&(c.toString=()=>b),c))}},has:(a,b)=>"data"!=b}):g})(Bliss,Mavo.value);
(function(a,b,c,d=c.util){function e(a,b){var c=a.toLocaleString(Mavo.locale,b);return c=c.replace(/\u200e/g,""),c}function f(a){return function(b){if(b=d.date(b),!b)return"";var c=h[a](b);return c=new self["year"==a?"String":"Number"](c),("month"==a||"weekday"==a)&&(c.name=e(b,{[a]:"long"}),c.shortname=e(b,{[a]:"short"})),c}}a.extend(c,{get $now(){return new Date},$startup:new Date,get $today(){return c.date(new Date)},year:f("year"),month:f("month"),day:f("day"),weekday:f("weekday"),hour:f("hour"),minute:f("minute"),second:f("second"),ms:f("ms"),date:(a)=>{return a=d.date(a),a?`${c.year(a)}-${c.digits(2,c.month(a))}-${c.digits(2,c.day(a))}`:""},time:(a)=>{return a=d.date(a),a?`${c.digits(2,c.hour(a))}:${c.digits(2,c.minute(a))}:${c.digits(2,c.second(a))}`:""},localTimezone:-new Date().getTimezoneOffset()});var g={seconds:1,minutes:60};for(let e in g.hours=60*g.minutes,g.days=24*g.hours,g.weeks=7*g.days,g.months=30.4368*g.days,g.years=12*g.months,c.msTo=(a,b)=>Math.floor(Math.abs(b)/(1e3*g[a]))||0,g)c[e]=(a)=>c.msTo(e,a);a.extend(c.util,{fixDateString:function(a){a=a.trim();var b=/^\d{4}-\d{2}(-\d{2})?/.test(a),d=-1<a.indexOf(":");return b||d?(a=b?a.replace(/^(\d{4}-\d{2})(?!-\d{2})/,"$1-01"):c.$today+" "+a,d?a=a.replace(/\-(\d{2})\s+(?=\d{2}:)/,"-$1T"):a+="T00:00:00",a=a.replace(/\s+/g,""),a):null},date:function(c){if(c=b(c),!c)return null;var e=new Date(c);if("string"!==a.type(c)||!isNaN(e)&&e+""==c)return e;if(c=d.fixDateString(c),null===c)return null;var f=Mavo.match(c,/[+-]\d{2}:?\d{2}|Z$/);if(f)c=new Date(c);else{var g=c.match(/\d+/g);c=new Date(g[0],(g[1]||1)-1,g[2]||1,g[3]||0,g[4]||0,g[5]||0,g[6]||0)}return isNaN(c)?null:c}});var h={year:(a)=>a.getFullYear(),month:(a)=>a.getMonth()+1,day:(a)=>a.getDate(),weekday:(a)=>a.getDay()||7,hour:(a)=>a.getHours(),minute:(a)=>a.getMinutes(),second:(a)=>a.getSeconds(),ms:(a)=>a.getMilliseconds()}})(Bliss,Mavo.value,Mavo.Functions);
(function(a,c,d){var e=Mavo.Script={addUnaryOperator:function(a,b){return b.symbol&&Mavo.toArray(b.symbol).forEach((b)=>{e.unarySymbols[b]=a,jsep.addUnaryOp(b)}),Mavo.Functions[a]=(a)=>e.unaryOperation(a,(a)=>b.scalar(c(a)))},unaryOperation:function(a,b){return Array.isArray(a)?a.map(b):b(a)},binaryOperation:function(c,a,b={}){return result=Array.isArray(a)?Array.isArray(c)?[...a.map((a,d)=>b.scalar(void 0===c[d]?b.identity:c[d],a)),...c.slice(a.length)]:a.map((a)=>b.scalar(c,a)):Array.isArray(c)?c.map((c)=>b.scalar(c,a)):b.scalar(c,a),result},addBinaryOperator:function(a,f){return f.symbol&&Mavo.toArray(f.symbol).forEach((b)=>{e.symbols[b]=a,f.precedence&&jsep.addBinaryOp(b,f.precedence)}),f.identity=void 0===f.identity?0:f.identity,Mavo.Functions[a]=f.code||function(...g){1===g.length&&Array.isArray(g[0])&&(g=[...g[0]]),f.raw||(g=g.map(c));var h,j=f.logical?f.identity:g[0];for(let c=1;c<g.length;c++){let i=f.logical?g[c-1]:j,a=g[c];Array.isArray(a)&&"number"==typeof f.identity&&(a=d.numbers(a)),h=e.binaryOperation(i,a,f),j=f.reduce?f.reduce(j,h,i,a):f.logical?j&&h:h}return j}},symbols:{},unarySymbols:{},getOperatorName:(a,b)=>e[b?"unarySymbols":"symbols"][a]||a,operators:{not:{symbol:"!",scalar:(b)=>!b},multiply:{scalar:(c,a)=>c*a,identity:1,symbol:"*"},divide:{scalar:(c,a)=>c/a,identity:1,symbol:"/"},addition:{scalar:(c,d)=>+c+ +d,symbol:"+"},plus:{scalar:(b)=>+b,symbol:"+"},subtract:{scalar:(c,a)=>{if(isNaN(c)||isNaN(a)){var b=d.date(c),e=d.date(a);if(b&&e)return b-e}return c-a},symbol:"-"},minus:{scalar:(b)=>-b,symbol:"-"},mod:{scalar:(c,a)=>{var b=c%a;return b+=0>b?a:0,b},symbol:"mod",precedence:6},lte:{logical:!0,scalar:(c,a)=>{return[c,a]=e.getNumericalOperands(c,a),c<=a},identity:!0,symbol:"<="},lt:{logical:!0,scalar:(c,a)=>{return[c,a]=e.getNumericalOperands(c,a),c<a},identity:!0,symbol:"<"},gte:{logical:!0,scalar:(c,a)=>{return[c,a]=e.getNumericalOperands(c,a),c>=a},identity:!0,symbol:">="},gt:{logical:!0,scalar:(c,a)=>{return[c,a]=e.getNumericalOperands(c,a),c>a},identity:!0,symbol:">"},eq:{logical:!0,scalar:(c,a)=>c==a,symbol:["=","=="],identity:!0,precedence:6},neq:{logical:!0,scalar:(c,a)=>c!=a,symbol:["!="],identity:!0},and:{logical:!0,scalar:(c,d)=>!!c&&!!d,identity:!0,symbol:["&&","and"],precedence:2},or:{logical:!0,scalar:(c,a)=>c||a,reduce:(a,b)=>a||b,identity:!1,symbol:["||","or"],precedence:2},concatenate:{symbol:"&",identity:"",scalar:(c,a)=>""+(c||"")+(a||""),precedence:10},object:{symbol:":",code:(...a)=>{for(var b=a.length-1,c=a[b];b--;)c={[a[b]]:c};return c},transformation:(a)=>{"Identifier"==a.left.type&&(a.left={type:"Literal",value:a.left.name,raw:JSON.stringify(a.left.name)})},precedence:4},filter:{symbol:"where",scalar:(d,a)=>c(a)?d:null,raw:!0,precedence:2,postFlattenTransformation:(a)=>{var b=a.arguments[0];for(let c=1;c<a.arguments.length;c++)e.walk(a.arguments[c],(a)=>{return{type:"MemberExpression",computed:!1,object:b,property:{type:"identifier",name:a.name}}},{type:"Identifier",ignore:["property","callee"]},c,a.arguments)}}},getNumericalOperands:function(c,a){if(isNaN(c)||isNaN(a)){var b=d.date(c),e=d.date(a);if(b&&e)return[b,e]}return[c,a]},childProperties:["arguments","argument","callee","left","right","elements","test","consequent","alternate","object","body"],walk:function(a,b,c={},d,f){if(!c.type||a.type===c.type)var g=b(a,d,f);return c.ignore&&-1!==c.ignore.indexOf(a.type)||e.childProperties.forEach((d)=>{a[d]&&e.walk(a[d],b,c,d,a)}),void 0!==g&&f&&(f[d]=g),g},serializers:{BinaryExpression:(a)=>`${e.serialize(a.left)} ${a.operator} ${e.serialize(a.right)}`,UnaryExpression:(a)=>`${a.operator}${e.serialize(a.argument)}`,CallExpression:(a)=>{if("MemberExpression"==a.callee.type)var b=", "+e.serialize(a.callee.object);var c=e.serialize(a.callee);if("Identifier"==a.callee.type){var d=[Array,String,Number].some((b)=>a.callee.name in b.prototype);d&&(c=`Mavo.Functions.${c.toLowerCase()} || ${c}`)}return`call(${c}, [${a.arguments.map(e.serialize).join(", ")}]${b||""})`},ConditionalExpression:(a)=>`${e.serialize(a.test)}? ${e.serialize(a.consequent)} : ${e.serialize(a.alternate)}`,MemberExpression:(a)=>{var b=a.computed?e.serialize(a.property):`"${a.property.name}"`;return`get(${e.serialize(a.object)}, ${b})`},ArrayExpression:(a)=>`[${a.elements.map(e.serialize).join(", ")}]`,Literal:(a)=>a.raw,Identifier:(a)=>a.name,ThisExpression:()=>"this",Compound:(a)=>a.body.map(e.serialize).join(", ")},transformations:{BinaryExpression:(a)=>{let b=e.getOperatorName(a.operator),c=e.operators[b];c.transformation&&c.transformation(a);var d=a,f={type:"CallExpression",arguments:[],callee:{type:"Identifier",name:b}};do f.arguments.unshift(d.right),d=d.left;while(!1!==c.flatten&&e.getOperatorName(d.operator)===b);return f.arguments.unshift(d),c.postFlattenTransformation&&c.postFlattenTransformation(f),f},UnaryExpression:(a)=>{var b=e.getOperatorName(a.operator,!0);if(b)return{type:"CallExpression",arguments:[a.argument],callee:{type:"Identifier",name:b}}},CallExpression:(a)=>{if("Identifier"==a.callee.type)if("if"==a.callee.name){a.callee.name="iff";var b=a.arguments[0];for(let c=1;c<a.arguments.length;c++)2==c&&(b=e.parse("not()"),b.arguments.push(a.arguments[0])),e.walk(a.arguments[c],(a)=>{var c=a.callee.name;Mavo.Actions.Functions.hasOwnProperty(c)&&!/if$/.test(c)&&(a.callee.name+="if",a.arguments.unshift(b))},{type:"CallExpression"})}else"delete"==a.callee.name&&(a.callee.name="clear")}},serialize:(a)=>{var b=e.transformations[a.type]&&e.transformations[a.type](a);if("object"==typeof b&&b&&b.type)a=b;else if(void 0!==b)return b;return e.serializers[a.type](a)},rewrite:function(a){try{return e.serialize(e.parse(a))}catch(b){return a}},compile:function(a,b={}){return a=e.rewrite(a),a=`with(Mavo.Functions._Trap)
	with (data || {}) {
		return (${a});
	}`,b.actions&&(a=`
...
${a}
Mavo.Actions.running = Mavo.Actions._running;`),new Function("data",a)},parse:self.jsep};for(let b in e.serializers.LogicalExpression=e.serializers.BinaryExpression,e.transformations.LogicalExpression=e.transformations.BinaryExpression,e.operators){let a=e.operators[b];a.scalar&&2>a.scalar.length?e.addUnaryOperator(b,a):e.addBinaryOperator(b,a)}var b={average:"avg",iff:"iff IF",multiply:"mult product",divide:"div",lt:"smaller",gt:"larger bigger",eq:"equal equality",ordinal:"th"};for(let e in b)b[e].split(/\s+/g).forEach((a)=>Mavo.Functions[a]=Mavo.Functions[e])})(Bliss,Mavo.value,Mavo.Functions.util);
(function(a){Mavo.attributes.push("mv-action");var b=Mavo.Actions={listener:(a)=>{var c=a.target.closest("[mv-action]"),d=Mavo.Node.get(c);d&&d.editing||c&&b.run(c.getAttribute("mv-action"),c)},run:(a,b)=>{if(a){var c=Mavo.Node.getClosest(b);if(c){var d=new Mavo.Expression(a);return d.eval(c.getLiveData(),{actions:!0})}}},getNodes:(a)=>{var c=b.getNode(a);return c?[c]:Mavo.toArray(a).map((a)=>b.getNode(a)).filter((a)=>a!==void 0)},getNode:(a)=>{if(a instanceof Mavo.Node)return a;return a&&a[Mavo.toNode]?a[Mavo.toNode]:void 0},Functions:{add:(a,c,d)=>{if(a){var e=b.getNode(a);if(!(e instanceof Mavo.Collection)){if(c){var f=b.getNode(c);f instanceof Mavo.Collection&&(e=f,c=a)}if(!(e instanceof Mavo.Collection)&&e&&e.collection){var g=e;e=e.collection,void 0===d&&(d=g.index)}if(!(e instanceof Mavo.Collection))return console.warn("The first parameter of add() needs to be a collection or collection item."),c}return(Array.isArray(c)?c:[c]).map((a)=>{var b=e.add(void 0,d);return void 0!==a&&b.render(a),e.editing&&e.editItem(b),b.getLiveData()})}},move:(c,d,e)=>{if(c&&void 0!==d){var f=b.getNode(d);"number"!=a.type(d)||f&&f.collection||(e=d,d=void 0);var g=Mavo.toArray(c).map(b.getNode).filter((a)=>a&&a.closestCollection);if(!g.length)return console.warn("The first parameter of move() should be a collection or collection item. There is nothing to move here."),c;var h=(f||g[0]).closestCollection,i=b.Functions.add(h,c,e);return Mavo.Collection.delete(g,{silent:!0}),i}},clear:(...a)=>{if(a.length&&a[0]){var c=b.getNodes(Mavo.flatten(a)),d=[];return c.forEach((a)=>{a&&(a instanceof Mavo.Collection?d.push(...a.children):a.collection?d.push(a):a.walk((c)=>{c instanceof Mavo.Primitive?c.value=null:c!==a&&b.clear(c)}))}),Mavo.Collection.delete(d),c.map((a)=>a.getLiveData())}},clearif:(a,...c)=>{return c=c.map((b)=>Mavo.Functions.iff(a,b)),b.Functions.clear(...c)},set:(a,c)=>{if(a){var d=Array.isArray(a),e=b.getNodes(a);return e.length?Mavo.Script.binaryOperation(d?e:e[0],c,{scalar:(a,b)=>a.render(b)}):console.warn(`The first parameter of set() needs to be one or more existing properties, ${Mavo.safeToJSON(a)} is not.`),c}}}};for(let c in b.Functions){let a=c+"if";a in b.Functions||(b.Functions[a]=(a,d,...e)=>{return d=Mavo.Functions.iff(a,d),b.Functions[c](d,...e)})}b.Functions.deleteif=b.Functions.clearif})(Bliss,Bliss.$);
